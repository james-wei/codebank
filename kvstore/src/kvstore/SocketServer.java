...
public class SocketServer {
    ...
    public void start() throws IOException {
        ...
        server.close();
        if (handler != null) {
            if (handler instanceof ServerClientHandler) {
                ((ServerClientHandler) handler).close();
            } else if (handler instanceof TPCClientHandler) {
                ((TPCClientHandler) handler).close();
            } else if (handler instanceof TPCMasterHandler) {
                ((TPCMasterHandler) handler).close();
            } else if (handler instanceof TPCRegistrationHandler) {
                ((TPCRegistrationHandler) handler).close();
            }
        }
    }
