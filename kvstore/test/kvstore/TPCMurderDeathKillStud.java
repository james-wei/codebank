...
public class TPCMurderDeathKillStud {
    ...
    KVMessage p2Retry = new KVMessage(KVConstants.PUT_REQ);
    ...
    public void setUp() throws Exception {
        ...
        p2Retry.setKey("6666666666666666670");
        p2Retry.setValue("Riyaz from the ashes");
    ...
    protected void startMockSlave(long slaveID, int deathOption) throws Exception {
        ...
        case 2:
            doAnswer(dieAfterLog).when(log).appendAndFlush(argThat(new isPutDel2())); // kill in phase 2 before response is sent
            break;

        case 3:
            doAnswer(dieAfterLog).when(log).appendAndFlush(argThat(new isDel()));     // kill in phase 1 after log
            break;

        case 4:
            doAnswer(dieBeforeLog).when(log).appendAndFlush(argThat(new isPutDel1())); //kill in phase 1 before log
            break;

        case 5:
            doAnswer(dieBeforeLog).when(log).appendAndFlush(argThat(new isPutDel2())); // kill in phase 2 before response is sent
            break;

        case 6:
            doAnswer(dieBeforeLog).when(log).appendAndFlush(argThat(new isDel()));     // kill in phase 1 before log
            break;
    ...
    //Used to allow the spied log to recognize if the message is a phase 2 commit. Don't care about aborts.
    class isDel extends ArgumentMatcher<KVMessage>{

        @Override
        public boolean matches(Object msg) {
            KVMessage m = (KVMessage) msg;
            if (m.getMsgType().equals(KVConstants.DEL_REQ)) return true;
            return false;
        }
    }
    ...
    @Test(timeout = 30000)
    public void testP2RetryAfterLog() {
        // Variation of testP1DeathAfterLog. Slave fails immediately after logging the commit.
        // The master expects a response, so it keeps retrying the commit until it receives a response.
        try{startMockSlave(SLAVE1, 2);} catch (Exception e) {fail("can't start slave");}
        try{
            master.handleTPCRequest(p2Retry, true);
        } catch (KVException e){
            fail("During phase 2, the master must retry until it receives a response");
        }
        checkBuild();
        try{
            assertEquals(slave1.get("6666666666666666670"), "Riyaz from the ashes");
        }
        catch(KVException e){
    ...
    public void testDelDeathAfterLog() {
        // Variation of testP1DeathAfterLog. Slave fails immediately after logging DEL request.
        // The master expects a response, so it keeps retrying the commit until it receives a response.
        ...
        try{
            master.handleTPCRequest(delDeath, false);
            fail("Shouldn't succeed");
        } catch (KVException e){
            //assertEquals(e.getKVMessage().getMessage(), KVConstants.ERROR_SOCKET_TIMEOUT);
        }
        checkBuild();
        try{
            ...
            fail("Key was deleted when it should have failed.");
        }
    ...
    @Test(timeout = 30000)
    public void testP1DeathBeforeLog(){
        try{startMockSlave(SLAVE1, 4);} catch (Exception e) {fail("can't start slave");}
        try{
            master.handleTPCRequest(p1Death, true);
            fail("Shouldn't succeed");
        } catch (KVException e){
    ...
    @Test(timeout = 30000)
    public void testP2RetryBeforeLog() {
        // Variation of testP1DeathAfterLog. Slave fails immediately after logging the commit.
        // The master expects a response, so it keeps retrying the commit until it receives a response.
        try{startMockSlave(SLAVE1, 5);} catch (Exception e) {fail("can't start slave");}
        try{
            master.handleTPCRequest(p2Retry, true);
        } catch (KVException e){
            fail("During phase 2, the master must retry until it receives a response");
    ...
    @Test(timeout = 30000)
    public void testDelDeathBeforeLog() {
        // Variation of testP1DeathAfterLog. Slave fails immediately after logging DEL request.
        // The master expects a response, so it keeps retrying the commit until it receives a response.
        try{startMockSlave(SLAVE1, 6);} catch (Exception e) {fail("can't start slave");}
        try{
            master.handleTPCRequest(p1Death, true);
        } catch (KVException e){
            fail("Shouldn't have thrown exception.");
